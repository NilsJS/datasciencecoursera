...
```{r echo = TRUE}
m1 <- lm(gpm ~ am, data=mtcars2)
m2 <- lm(gpm ~ am + wt, data=mtcars2)
m3 <- lm(gpm ~ am + wt + hp, data=mtcars2)
m4 <- lm(gpm ~ am + wt + hp + disp, data=mtcars2)
m5 <- lm(gpm ~ am + wt + hp + disp + gear, data=mtcars2)
m6 <- lm(gpm ~ am + wt + hp + disp + gear + carb, data=mtcars2)
m7 <- lm(gpm ~ am + wt + hp + disp + gear + carb + cyl, data=mtcars2)
m8 <- lm(gpm ~ am + wt + hp + disp + gear + carb + cyl + drat, data=mtcars2)
m9 <- lm(gpm ~ am + wt + hp + disp + gear + carb + cyl + drat + qsec, data=mtcars2)
m10 <- lm(gpm ~ am + wt + hp + disp + gear + carb + cyl + drat + qsec + vs, data=mtcars2)
```

```{r echo = FALSE}
a <- anova(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10)
panderOptions('table.split.table', Inf) 
pander(a, style="rmarkdown", caption="anova results")
```

It seems from *anova* that models 2, 3 were significant. 

```{r echo = FALSE}
# Compare the impact on am for each of the models
...
panderOptions('table.split.table', Inf) 
pander(cmp, style="rmarkdown", caption="Impact on the am predictor for the most significant models")
```
...
#### Model selection, using * rather than '+'
...
Onlymode 2 & 3 (wt + hp) were significant
...
panderOptions('table.split.table', Inf) 
pander(cmp, style="rmarkdown", caption="Impact on the am predictor for each of the 10 models")
```
