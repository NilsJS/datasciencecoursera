...
**However:** adding more factors to our model has also increased the *p-value* for am to `r round(coef3[2,4], 3)` so that it now exceeds the customary cutof of 0.05.  **We therefore cannot reject $H_0$ using this model.**  The confidence interval for the slope of the am predictor  (`r round(am_ci[1], 4)` ;  `r round(am_ci[2], 4)`) includes 0 which means it may have no impact.  The residual variability $\sigma$ is `r round(summary(fit3)$sigma, 4)`, and the residual plot in the appendix also shows that there is quite a wide spread, particularly for automatic.  This indicates that our model is a bad fit or that we simply do not have enough data in our sample.
...
```{r echo = FALSE, fig.height = 4, fig.width=automatic, fig.cap='Model3: mpg ~ am + hp + wt'}
...
```
```{r echo = FALSE, fig.height = 4, fig.width=automatic, fig.cap='Residual plot for model 3'}
ggplot(mtcars, aes(x=hp, y=Residual, size = wt, color = am)) +
  geom_point(alpha=.5) + 
