...
```{r Reading full Engligh Corpus, cache=F}
...
According to wikipedia, the (corpus of the english language)[https://en.wikipedia.org/wiki/Most_common_words_in_English] has more than **a billion words**.  

Our dictionary has `r length(ft)` distinct words, based on a corpus of `r sum(ft.dt$total)` words in total (the complete text).  The top 100 words covers `r `
